		...
		try{
			...
			while (bufferedBytes > 0) {
				if (endedPacket) {
					...
					if (packetId < PacketsShit.fields.length) packet = PacketsShit.fields[packetId];
					else{
						...
						try{ Chat.OutputChatMessage('Recieved abnormal packet with id $packetId?');
						}catch(e){}
	...
	public function readBuffer(n:Int, bytes:Int) {
		...
		for (buf in buffers) {
			...
			if (n < runningTotal) {
				...
				result = (bytes == 2 ? buf.readUnsignedShort() : buf.readUnsignedInt());
				buf.position = 0;

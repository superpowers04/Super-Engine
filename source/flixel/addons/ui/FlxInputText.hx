...
class FlxInputText extends FlxText {
	...
	public var caretWidth(default, set):Int = 2; // Set to 2 instead of 1, since bug causes caret to disappear sometimes
	...
	private function set_hasFocus(newFocus:Bool):Bool {
		if (newFocus) {
			...
			CoolUtil.updateActiveObject(this);
		} else {
			// Graphics
			caret.visible = false;
			if (_caretTimer != null) {
